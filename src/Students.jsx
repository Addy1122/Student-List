...
setstudent(newitems)
    ...
    return(<div>{
        studentarr.map(function(item,index){
            return <div className="flex justify-between w-80 text-2xl m-2 ">{index+1}.{item.name}
            <button className="bg-black-700 text-white p-1  border rounded-lg text-xl" onClick={()=>addstudent(item.name,index)}  disabled={item.disabled} style={{ backgroundColor: item.disabled ? 'grey' : 'black'  }} >Add to favourites</button></div>
        })
        }</div>)
  }
